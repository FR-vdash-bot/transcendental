...
lemma of_subring_to_subring (hp : (↑p.frange : set R) ⊆ T) :
  (p.to_subring T hp).of_subring T = p := by
{ dsimp only [of_subring, to_subring],
--  simp_rw [← polynomial.sum_def],
}

end to_subring

lemma F'_eq_prod {q : ℕ} (q0 : 0 < q) (p : ℤ[X])
  {s : ℂ} (hs : s ∈ (p.map (algebra_map ℤ ℂ)).roots) :
  F' q0 p hs =
  (((p.map (algebra_map ℤ ℂ)).roots.erase s).map
    (λ (a : ℂ), X - C (mk_smul_root p a))).prod ^ q *
      (X - C (mk_smul_root p s)) ^ (q - 1) := by
{ simp_rw [F', F_eq_of_subring q0 p hs, of_subring, to_subring],
  
}

lemma eval_derivative (p : ℂ[X]) {s : ℂ} (hs : s ∈ p.roots) :
  p.derivative.eval s =
    p.leading_coeff * ((p.roots.erase s).map (λ (a : ℂ), X - C a)).prod.eval s := by
{ have : p.roots.card = p.nat_degree := splits_iff_card_roots.mp (is_alg_closed.splits _),
  conv_lhs { rw [← C_leading_coeff_mul_prod_multiset_X_sub_C this], },
  rw [← @multiset.prod_erase _ _ _ (X - C s)_ (multiset.mem_map.mpr ⟨s, ⟨hs, rfl⟩⟩)],
  simp_rw [← smul_eq_C_mul, derivative_smul, derivative_mul, smul_add, eval_add, eval_smul,
    eval_mul, eval_sub, eval_X, eval_C, sub_self, zero_mul, smul_zero, add_zero,
    derivative_sub, derivative_X, derivative_C, sub_zero, eval_one, one_mul, smul_eq_mul],
  rw [← multiset.map_erase],
  intros x y h, dsimp only at h, rwa [sub_right_inj, C_inj] at h, }

lemma sum_ideriv_F_eq (M : multiset (ℤ × ℤ[X])) {q : ℕ} (q0 : 0 < q)
  (m0 : ∀ (m : ℤ × ℤ[X]), m ∈ M → m.2 ≠ 0)
  {m : ℤ × ℤ[X]} (hm : m ∈ M) {s : ℂ} (hs : s ∈ (m.2.map (algebra_map ℤ ℂ)).roots) :
  ∃ (n' : integral_closure ℤ ℂ), (F q m.2 s).sum_ideriv.eval (↑(m.2.leading_coeff) * s) =
    (q - 1)! * ((m.2.map (algebra_map ℤ ℂ)).derivative.eval (↑(m.2.leading_coeff) * s)) ^ q +
    q! * n' := by
{ have m0' : m.snd.map (algebra_map ℤ ℂ) ≠ 0 := λ h, by
  { rw [← polynomial.map_zero (algebra_map ℤ ℂ)] at h,
    exact m0 m hm (polynomial.map_injective _ (algebra_map ℤ ℂ).injective_int h), },
  have aeval_s := (mem_roots m0').mp hs, rw [is_root, ← eval₂_eq_eval_map, ← aeval_def] at aeval_s,
  have : ∀ {h}, ∃ (p' : (integral_closure ℤ ℂ)[X]),
    F' q0 m.snd hs = (X - C (⟨m.snd.leading_coeff • s, h⟩ : integral_closure ℤ ℂ)) ^ q * p',
  { intros h,
    
    dsimp only [F'], rw [to_subring],
  },
  have := sum_ideriv_sl (F' q0 m.2 hs) ⟨m.2.leading_coeff • s,
    is_integral_leading_coeff_smul _ _ aeval_s⟩ q,
  
  let scale := λ (s : ℂ), ↑(m.snd.leading_coeff) * s,
  have := is_integral_leading_coeff_smul _ _ aeval_s, rw [zsmul_eq_mul] at this,
  have h := F_eq_prod hq m.2 hs, rw [mul_comm] at h,
  convert sum_ideriv_sl' _ this hq h using 0,
  funext n', congr',
  rw [eval_derivative _ hs, ← C_pow, eval_C_mul, mul_pow, eval_pow], }

lemma sum_P_F_eq (M : multiset (ℤ × ℤ[X])) {q : ℕ} (hq : 0 < q)
  (m0 : ∀ (m : ℤ × ℤ[X]), m ∈ M → m.2 ≠ 0)
  {m : ℤ × ℤ[X]} (hm : m ∈ M) {s : ℂ} (hs : s ∈ (m.2.map (algebra_map ℤ ℂ)).roots)
  (ht : (M.map (λ m : ℤ × ℤ[X],
    m.1 • ((m.2.map (algebra_map ℤ ℂ)).roots.map (λ r, exp r)).sum)).sum = 0) :
  ∃ n', (M.map (λ m' : ℤ × ℤ[X],
    m'.1 • ((m'.2.map (algebra_map ℤ ℂ)).roots.map (λ r, P (F q m s) r)).sum)).sum =
  (q - 1)! * -(M.map (λ m' : ℤ × ℤ[X],
    m'.1 • ((m'.2.map (algebra_map ℤ ℂ)).roots.map (λ r, (F q m s).eval r)).sum)).sum +
    q! * n' := by
{ let c : ℤ × ℤ[X] → ℂ → ℤ := ∀ (m' : ℤ × ℤ[X]) (r : ℂ),
    if hm' : m' ∈ M then
      if hr : r ∈ (m'.2.map (algebra_map ℤ ℂ)).roots then
        classical.some (sum_ideriv_F_eq M hq m0 hm' hr)
      else 0
    else 0,
  
  use 114514,
  rw [sum_P_eq _ _ m0 ht], congr', funext, congr', funext,
  
}

/-
def mk_smul_root (p : ℤ[X]) (x : ℂ) : integral_closure ℤ ℂ :=
  if h : x ∈ (p.map (algebra_map ℤ ℂ)).roots then ⟨p.leading_coeff • x,
    is_integral_leading_coeff_smul _ _ (aeval_of_mem_roots p h)⟩
  else 0

def F'_aux (q : ℕ) (p : ℤ[X]) (s : ℂ) : (integral_closure ℤ ℂ)[X] :=
  (((p.map (algebra_map ℤ ℂ)).roots.erase s).map
    (λ (a : ℂ), X - C (mk_smul_root p a))).prod ^ q *
      (X - C (mk_smul_root p s)) ^ (q - 1)

lemma F_eq_of_subring {q : ℕ} (q0 : 0 < q) (p : ℤ[X])
  {s : ℂ} (hs : s ∈ (p.map (algebra_map ℤ ℂ)).roots) :
  F q p s = of_subring (integral_closure ℤ ℂ).to_subring (F'_aux q p s) := by
{ have map_mk_smul_root : ∀ {x : ℂ} (hx : x ∈ (p.map (algebra_map ℤ ℂ)).roots),
    (algebra_map (integral_closure ℤ ℂ) ℂ) (mk_smul_root p x) = p.leading_coeff * x,
  { intros x hx, dsimp only [mk_smul_root], split_ifs,
    rw [subalgebra.algebra_map_eq, algebra.id.map_eq_id, ring_hom_comp_triple.comp_eq,
      alg_hom.coe_to_ring_hom, subalgebra.coe_val, ← zsmul_eq_mul], refl, },
  have : function.injective (algebra_map (integral_closure ℤ ℂ) ℂ),
  { exact no_zero_smul_divisors.algebra_map_injective _ _, },
  rw [F_eq_prod q0 _ hs, as_sum_support (_ * _)],
  dsimp only [of_subring, F'_aux], apply sum_congr,
  { simp_rw [← support_map_of_injective _ this, polynomial.map_mul, polynomial.map_pow,
      polynomial.map_multiset_prod, polynomial.map_sub, map_X, map_C, map_mk_smul_root hs,
      multiset.map_map, function.comp],
    congr' 4, apply multiset.map_congr rfl, intros x hx,
    rw [polynomial.map_sub, map_X, map_C, map_mk_smul_root (multiset.mem_of_mem_erase hx)], },
  intros x hx,
  convert_to _ = monomial x ((algebra_map (integral_closure ℤ ℂ) ℂ)
    (((multiset.map (λ (a : ℂ), X - C (mk_smul_root p a))
    ((map (algebra_map ℤ ℂ) p).roots.erase s)).prod ^ q *
    (X - C (mk_smul_root p s)) ^ (q - 1)).coeff x)),
  simp_rw [← coeff_map, polynomial.map_mul, polynomial.map_pow, polynomial.map_multiset_prod,
    polynomial.map_sub, map_X, map_C, map_mk_smul_root hs, multiset.map_map, function.comp],
  congr' 5, apply multiset.map_congr rfl, intros x hx,
  rw [polynomial.map_sub, map_X, map_C, map_mk_smul_root (multiset.mem_of_mem_erase hx)], }

lemma frange_F_mem_integral_closure {q : ℕ} (q0 : 0 < q) (p : ℤ[X])
  {s : ℂ} (hs : s ∈ (p.map (algebra_map ℤ ℂ)).roots) :
  ((F q p s).frange : set ℂ) ⊆ integral_closure ℤ ℂ :=
by { rw [F_eq_of_subring q0 p hs], exact frange_of_subring _, }

def F' {q : ℕ} (q0 : 0 < q) (p : ℤ[X])
  {s : ℂ} (hs : s ∈ (p.map (algebra_map ℤ ℂ)).roots) :
  (integral_closure ℤ ℂ)[X] :=
to_subring (F q p s) (integral_closure ℤ ℂ).to_subring (frange_F_mem_integral_closure q0 p hs)
...
{ have m0' : m.snd.map (algebra_map ℤ ℂ) ≠ 0 := λ h, by
  ...
  funext n', congr',
  rw [eval_derivative _ hs, ← C_pow, eval_C_mul, mul_pow, eval_pow], }
#check ring_of_integrals
#check number_field.ring_of_integrals
#check integral_closure
#check ring_hom.is_integral_elem_leading_coeff_mul
lemma sum_P_F_eq (M : multiset (ℤ × ℤ[X])) {q : ℕ} (hq : 0 < q)
  (m0 : ∀ (m : ℤ × ℤ[X]), m ∈ M → m.2 ≠ 0)
  {m : ℤ × ℤ[X]} (hm : m ∈ M) {s : ℂ} (hs : s ∈ (m.2.map (algebra_map ℤ ℂ)).roots)
  (ht : (M.map (λ m : ℤ × ℤ[X],
...
/-
def F (M : multiset (ℤ × ℤ[X])) (p : ℕ) (m : ℤ × ℤ[X]) (hm : m ∈ M)
  (s : ℂ) (hs : s ∈ (m.2.map (algebra_map ℤ ℂ)).roots) : ℂ[X] :=
((M.map (λ m : ℤ × ℤ[X], m.2)).prod ^ p).map (algebra_map ℤ ℂ) /ₘ (X - C s)
-/
